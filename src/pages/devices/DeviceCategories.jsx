...
export default function Categories() {
  ...
  // Dummy device counts (replace with real data if available)
  const getCategoryStats = (categoryId) => {
    const total = Math.floor(Math.random() * 20) + 5;
    const inUse = Math.floor(total * 0.6); // 60% in use
  ...
  return (
    <div className=" space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-blue-600">Devices Categories</h1>
          ...
          <tbody>
            {categories.length > 0 ? (
              categories.map((cat, index) => {
                const { total, inUse, notInUse } = getCategoryStats(cat.id);
                return (
                  <tr
                    key={cat.id}
